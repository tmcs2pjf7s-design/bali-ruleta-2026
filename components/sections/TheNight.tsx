...
import { NIGHT } from '@/data/experience'
  ...
  return (
    <section id="night" className="scroll-mt-24 border-t border-line px-5 py-24 md:px-10 md:py-40">
      <div className="mx-auto max-w-[1400px]">
        <Reveal><Eyebrow index="06">The Four-Hour Journey</Eyebrow></Reveal>
        <Reveal as="h2" delay={80} className="mt-10 max-w-3xl font-display text-3xl font-light leading-tight text-ivory md:text-5xl">
          One evening, told in order.
        </Reveal>

        <ol className="mt-16 max-w-2xl">
          {NIGHT.map((b, i) => (
            <Reveal as="li" key={b.label} delay={i * 40} className="relative border-l border-line pl-8 pb-10 last:pb-0">
              ...
              <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                ...
                <span className="font-display text-2xl font-light text-ivory">{b.label}</span>
              </div>
              {b.note && <p className="mt-1.5 text-[0.85rem] text-mist">{b.note}</p>}
            </Reveal>
          ))}
        </ol>
      </div>
