  ...
  return (
    <section id="location" className="scroll-mt-24 border-t border-line px-5 py-24 md:px-10 md:py-40">
      <div className="mx-auto max-w-[1400px]">
        ...
        <Reveal className="mt-8 max-w-lg space-y-4 text-sm leading-relaxed text-graphite md:text-[0.95rem]">
          {LOCATION.body.map(p => <p key={p.en}>{t(p, lang)}</p>)}
          <p className="pt-2 text-[0.7rem] uppercase tracking-widest2 text-warmgrey">{t(LOCATION.city, lang)}</p>
        </Reveal>
      </div>
    </section>
  )
